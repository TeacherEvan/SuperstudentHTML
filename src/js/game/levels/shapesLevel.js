...
export default class ShapesLevel extends BaseLevel {
  constructor(canvas, ctx, managers, helpers) {
    ...
    this.sequence = this.isE2EMode()
      ? GAME_CONFIG.SEQUENCES.shapes.slice(0, 2)
      : GAME_CONFIG.SEQUENCES.shapes;
    this.objects = [];
    this.currentIndex = 0;
    this.groupCount = 0;
    this.spawnTimer = 0;
  ...
  getTestSnapshot() {
    return {
      ...
      targets: this.objects
        .filter((obj) => obj.shape === this.currentTarget)
        .map((obj, index) => this.toNormalizedPoint(obj.x, obj.y, { index, label: obj.shape }))
    };
  }
